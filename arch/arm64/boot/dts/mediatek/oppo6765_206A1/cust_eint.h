/*
 * Generated by MTK SP DrvGen Version: 3.5.160809 for MT6765.
 * 2026-08-02 08:23:38
 * Do Not Modify The File.
 * Copyright Mediatek Inc. (c) 2016.
*/

#ifndef __CUST_EINT_H
#define __CUST_EINT_H

#ifdef __cplusplus
extern "C" {
#endif
#define CUST_EINTF_TRIGGER_RISING			1
#define CUST_EINTF_TRIGGER_FALLING			2
#define CUST_EINTF_TRIGGER_HIGH			4
#define CUST_EINTF_TRIGGER_LOW			8
#define CUST_EINT_DEBOUNCE_DISABLE			0
#define CUST_EINT_DEBOUNCE_ENABLE			1


#define CUST_EINT_TOUCH_NUM			0
#define CUST_EINT_TOUCH_DEBOUNCE_CN		0
#define CUST_EINT_TOUCH_TYPE			CUST_EINTF_TRIGGER_FALLING
#define CUST_EINT_TOUCH_DEBOUNCE_EN		CUST_EINT_DEBOUNCE_DISABLE

#define CUST_EINT_MSDC1_INS_NUM			1
#define CUST_EINT_MSDC1_INS_DEBOUNCE_CN		0
#define CUST_EINT_MSDC1_INS_TYPE			CUST_EINTF_TRIGGER_RISING
#define CUST_EINT_MSDC1_INS_DEBOUNCE_EN		CUST_EINT_DEBOUNCE_DISABLE

#define CUST_EINT_MRDUMP_EXT_RST_NUM			2
#define CUST_EINT_MRDUMP_EXT_RST_DEBOUNCE_CN		512
#define CUST_EINT_MRDUMP_EXT_RST_TYPE			CUST_EINTF_TRIGGER_LOW
#define CUST_EINT_MRDUMP_EXT_RST_DEBOUNCE_EN		CUST_EINT_DEBOUNCE_ENABLE

#define CUST_EINT_EGIS_FP_NUM			5
#define CUST_EINT_EGIS_FP_DEBOUNCE_CN		0
#define CUST_EINT_EGIS_FP_TYPE			CUST_EINTF_TRIGGER_FALLING
#define CUST_EINT_EGIS_FP_DEBOUNCE_EN		CUST_EINT_DEBOUNCE_DISABLE

#define CUST_EINT_IRQ_NFC_NUM			10
#define CUST_EINT_IRQ_NFC_DEBOUNCE_CN		0
#define CUST_EINT_IRQ_NFC_TYPE			CUST_EINTF_TRIGGER_HIGH
#define CUST_EINT_IRQ_NFC_DEBOUNCE_EN		CUST_EINT_DEBOUNCE_DISABLE

#define CUST_EINT_SUBPMIC_PMU_EINT_NUM			11
#define CUST_EINT_SUBPMIC_PMU_EINT_DEBOUNCE_CN		0
#define CUST_EINT_SUBPMIC_PMU_EINT_TYPE			CUST_EINTF_TRIGGER_FALLING
#define CUST_EINT_SUBPMIC_PMU_EINT_DEBOUNCE_EN		CUST_EINT_DEBOUNCE_DISABLE

#define CUST_EINT_RT9465_SLAVE_CHR_NUM			23
#define CUST_EINT_RT9465_SLAVE_CHR_DEBOUNCE_CN		0
#define CUST_EINT_RT9465_SLAVE_CHR_TYPE			CUST_EINTF_TRIGGER_LOW
#define CUST_EINT_RT9465_SLAVE_CHR_DEBOUNCE_EN		CUST_EINT_DEBOUNCE_DISABLE

#define CUST_EINT_TCPC_PD_NUM			41
#define CUST_EINT_TCPC_PD_DEBOUNCE_CN		0
#define CUST_EINT_TCPC_PD_TYPE			CUST_EINTF_TRIGGER_FALLING
#define CUST_EINT_TCPC_PD_DEBOUNCE_EN		CUST_EINT_DEBOUNCE_DISABLE

#define CUST_EINT_DSI_TE_NUM			44
#define CUST_EINT_DSI_TE_DEBOUNCE_CN		0
#define CUST_EINT_DSI_TE_TYPE			CUST_EINTF_TRIGGER_RISING
#define CUST_EINT_DSI_TE_DEBOUNCE_EN		CUST_EINT_DEBOUNCE_DISABLE

#define CUST_EINT_SWTP_NUM			76
#define CUST_EINT_SWTP_DEBOUNCE_CN		256
#define CUST_EINT_SWTP_TYPE			CUST_EINTF_TRIGGER_HIGH
#define CUST_EINT_SWTP_DEBOUNCE_EN		CUST_EINT_DEBOUNCE_ENABLE

#define CUST_EINT_IRQ_NFC_NUM			97
#define CUST_EINT_IRQ_NFC_DEBOUNCE_CN		0
#define CUST_EINT_IRQ_NFC_TYPE			CUST_EINTF_TRIGGER_LOW
#define CUST_EINT_IRQ_NFC_DEBOUNCE_EN		CUST_EINT_DEBOUNCE_DISABLE

#define CUST_EINT_MAIN_PMIC_NUM			144
#define CUST_EINT_MAIN_PMIC_DEBOUNCE_CN		0
#define CUST_EINT_MAIN_PMIC_TYPE			CUST_EINTF_TRIGGER_HIGH
#define CUST_EINT_MAIN_PMIC_DEBOUNCE_EN		CUST_EINT_DEBOUNCE_DISABLE

#define CUST_EINT_OTG_IDDIG_NUM			147
#define CUST_EINT_OTG_IDDIG_DEBOUNCE_CN		0
#define CUST_EINT_OTG_IDDIG_TYPE			CUST_EINTF_TRIGGER_LOW
#define CUST_EINT_OTG_IDDIG_DEBOUNCE_EN		CUST_EINT_DEBOUNCE_DISABLE

#ifdef __cplusplus
}
#endif


#endif /* __CUST_EINT_H */
